...
function generateImageFile(outFile,paths){
    //clear the context (ctx) (previous drawings)
    ctx.clearRect(0,0,canvas.width,canvas.height); 
    draw.paths(ctx,paths); // make drawing  on context
    // convert canvas obj to PNG file format
    const buffer=canvas.toBuffer("image/png");
    fs.writeFileSync(outFile, buffer); // write to file system 
}